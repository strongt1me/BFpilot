...
main(void) {
  ...
  int uninstall_rc = BFPILOT_DIAG_SKIPPED;
  int final_rc = -1;

  installer_log("entered main direct-appinst tag=%s build=%s",
                VERSION_TAG, BUILD_VERSION);
  bfpilot_notify("BFpilot launcher installer", "direct AppInst installer");
  ...
  if(appinst_init_rc != 0) {
    installer_log("final result=failed_nonfatal final_rc=%d "
                  "appinst_init_rc=0x%08x", final_rc, appinst_init_rc);
  ...
  installer_log("about to sceAppInstUtilAppUnInstall %s",
                BFPILOT_APP_TITLE_ID);
  uninstall_rc = sceAppInstUtilAppUnInstall(BFPILOT_APP_TITLE_ID);
  installer_log("uninstall return code=0x%08x", uninstall_rc);
  ...
  app_install_title_dir_fn install_title_dir = resolve_title_dir();
  ...
  installer_log("final result=%s final_rc=%d appinst_init_rc=0x%08x "
                "uninstall_rc=0x%08x mkdir_app_rc=%d mkdir_sce_sys_rc=%d "
                "write_param_rc=%d write_icon_rc=%d title_dir_rc=0x%08x "
                "install_all_rc=0x%08x",
                final_rc == 0 ? "installed" : "failed_nonfatal", final_rc,
                appinst_init_rc, uninstall_rc, mkdir_app_rc,
                mkdir_sce_sys_rc, write_param_rc, write_icon_rc,
                title_dir_rc, install_all_rc);
